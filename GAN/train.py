...
from data import (TokenBucketSampler, PrefetchLoader,
                  ...
                  VqaDataset, VqaEvalDataset, VeEvalDataset, ImageLmdbGroup,
                  ve_collate, ve_eval_collate)
from discriminator import UniterModelforSequenceClassification
...
def main(opts):
    ...
    # def build_dbs_dataloader(txt_dbs, img_dbs, is_train, opts):
    #     datasets = []
    #     for txt_path, img_path in zip(txt_dbs, img_dbs):
    #         img_db = all_img_dbs[img_path]
    #         txt_db = TxtTokLmdb(txt_path, opts.max_txt_len)
    #         datasets.append(VqaDataset(txt_db, img_db))
    #     dataset = ConcatDatasetWithLens(datasets)
    #     dataloader = build_dataloader(dataset, ve_collate, is_train, opts)
    #     return dataloader
    
    # train_dataloader = build_dbs_dataloader(opts.train_txt_dbs, opts.train_img_dbs, True, opts)
    # val_dataloader = build_dbs_dataloader(opts.val_txt_dbs, opts.val_img_dbs, False, opts)
    
    train_datasets = []
    for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
        img_db = all_img_dbs[img_path]
        txt_db = TxtTokLmdb(txt_path, opts.max_txt_len)
        train_datasets.append(VqaDataset(2, txt_db, img_db))
    train_dataset = ConcatDatasetWithLens(train_datasets)
    train_dataloader = build_dataloader(train_dataset, ve_collate, True, opts)
    
    val_datasets = []
    for txt_path, img_path in zip(opts.val_txt_dbs, opts.val_img_dbs):
        img_db = all_img_dbs[img_path]
        txt_db = TxtTokLmdb(txt_path, opts.max_txt_len)
        val_datasets.append(VqaEvalDataset(2, txt_db, img_db))
    val_dataset = ConcatDatasetWithLens(val_datasets)
    val_dataloader = build_dataloader(val_dataset, ve_eval_collate, False, opts)
