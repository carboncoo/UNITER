...
def main(opts):
    ...
    val_log, results, logits = evaluate(model, eval_dataloader, label2ans,
                                        opts.save_logits)
    result_dir = f'{opts.txt_db}/results_test'
    ...
    if hvd.rank() == 0:
        avg_score = np.mean([float(x['answer']) for x in all_results])
        LOGGER.info(f"average score = {avg_score} for {len(all_results)} examples")
...
def evaluate(model, eval_loader, label2ans, save_logits=False):
    ...
    for i, batch in enumerate(eval_loader):
        ...
        #                                )[1].cpu().tolist()]
        # import ipdb; ipdb.set_trace()
        answers = ['{:.6f}'.format(s) for s in scores.softmax(dim=-1)[:, 1].cpu().tolist()]
